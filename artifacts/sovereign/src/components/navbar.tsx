...
export function Navbar() {
  ...
  return (
    <>
      ...
      {/* Secret / Admin button — floats just below the navbar on the right */}
      <div className="fixed top-16 right-4 sm:right-8 z-39 flex items-center gap-2 pt-1.5">
        {isAdmin ? (
          <>
            <button onClick={openAdmin}
              className="text-[9px] font-bold text-primary/70 hover:text-primary transition-colors uppercase tracking-widest border border-primary/20 hover:border-primary/50 px-2 py-1 rounded bg-black/60 backdrop-blur-sm">
              ADMIN
            </button>
            <button onClick={() => setIsAdmin(false)}
              className="text-[9px] font-bold text-red-500/50 hover:text-red-400 transition-colors uppercase tracking-widest cursor-pointer bg-black/60 backdrop-blur-sm px-2 py-1 rounded border border-red-900/20 hover:border-red-900/50">
              LOGOUT
            </button>
          </>
        ) : (
          <button onClick={openAdmin}
            className="text-[9px] font-bold text-white/10 hover:text-primary/50 transition-colors uppercase tracking-widest cursor-pointer">
            SECRET
          </button>
        )}
      </div>
